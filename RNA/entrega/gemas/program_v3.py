...
fpath=r'data/test_new/Amethyst/amethyst_18.jpg'
img=plt.imread(fpath)
print (img.shape)
imshow(img)

def preprocess (train_dir,test_dir, trsplit):
...
train_df, test_df, valid_df= preprocess(train_dir, test_dir, .8)
...
def balance(train_df,max_samples, min_samples, column, working_dir, image_size):
    ...
    gen=ImageDataGenerator(horizontal_flip=True, vertical_flip=True,  rotation_range=20, width_shift_range=.2,
                                  height_shift_range=.2, zoom_range=.2, brightness_range=(0.5, 0.9))
